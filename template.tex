...
\begin{equation} 
	...
	\right.
\end{equation}
where $du$ and $dv$ are the length of the $u$- and $v$-axis in image plane, optical point is $o(u_o,v_o)$. Redefine the current pan and tilt angle by $\phi_{pl}$ and $\psi_{pl}$, which can be obtained through the serial ports from the PTU directly. Then, the total composition pan and tilt angle is given by
\begin{equation} 
	\left \{
	\begin{split}
...
As derived in \cite{Marquez-Neila2014}, we set the level set function as $u(x, y, z, t)$ and its gradient as
\begin{align}
u_{\nabla} \triangleq \nabla u(x, y, z, t)
\end{align}
and furthermore
...
\begin{align}
	\frac{\partial u}{\partial t} = cg(I)\nu +cg(I)\text{div}(\frac{u_{\nabla}}{c}) + \nabla g(I) u_{\nabla}
\end{align}
...
\end{align}
where the $(D_du^n)(\mathbf{x})$ and $(E_du^n)(\mathbf{x})$ are dilation and erosion operator; $n$ is the curve evolution step and the gradient is defined as $ g_{\nabla} \triangleq \nabla g(I)(\mathbf{x})$. As the Eq.\ref{eq:PDE_1_3}, Eq.\ref{eq:PDE_2_3} and Eq.\ref{eq:PDE_3_3} illustrated, we divided the PDE calculation into three separated morphological steps. The SRC-TLD and BBS-based algorithm framework are shown in Alg.\ref{alg:active_contour}.
...
\end{align}
Fig.\ref{fig:chp04_17_predict_1} illustrates consecutive frames when the pitch angle jumped at the moment from the 138th to 139th frames during the flight experiment. Based on the changing pattern of UAV, without the PTU rotation status, a mistaken selection region is predicted by the original approach, shown as the yellow dot in Fig. \ref{fig:chp04_17_predict_1}. The blue dot on the same figure shows the proposed prediction method result. It is obvious that the blue dot position is much near to the green one which is the ground truth target center.
...
\begin{table*}[!th]
	...
	\begin{tabular}{cccc|cccc}
		\hline
		\textbf{Interval} & \textbf{$\mathbf{i}^{O,c}$} & \textbf{$\mathbf{j}^{O,c}$} & \textbf{$\mathbf{k}^{O,c}$} & \textbf{Interval} & \textbf{$\mathbf{i}^{O,c}$} & \textbf{$\mathbf{j}^{O,c}$} & \textbf{$\mathbf{k}^{O,c}$} \\ \hline
